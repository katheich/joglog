...
const RaceForm = ( { date, modalDate, toggleModal, race, updateInfo }) => {
  ...
  return <div className="has-text-centered">
    ...
    <form className="form form-home" onSubmit={handleSubmit}>
      ...
      {race ? <></> : <><Mutation mutation={POST_MUTATION} variables={{ ...info }} onCompleted={data => confirm(data.createRace, 'create')} onError={err => setErrors(err.message)}>
      ...
      {race ? <><Mutation mutation={EDIT_MUTATION} variables={{ ...info }} onCompleted={data => confirm(data.editRace, 'update')} onError={err => setErrors(err.message)}>
        {editMutation => <button onClick={editMutation} className="button is-primary"><i className="fas fa-check"></i></button>}
      </Mutation>
      <Mutation mutation={DELETE_MUTATION} variables={{ id: info.id }} onCompleted={data => confirm(data.deleteRace, 'delete')} onError={err => setErrors(err.message)}>
